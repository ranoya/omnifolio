...
?>
    <title>mouseClicked | O tecido das tecnologias criativas</title>
    <meta name="description" content="mouseClicked | O código Processing, capítulo do livro O tecido das tecnologias criativas">
    <meta property="og:title" content="mouseClicked | O tecido das tecnologias criativas">
    <meta property="og:description" content="mouseClicked | O código Processing, capítulo do livro O tecido das tecnologias criativas">    
...
<h1 id='titulocapitulo'>mouseClicked()</h1>

  <p>A função <code>mouseClicked()</code> é uma função invocada pelo contexto do <span class="marca">Processing</span> quando um evento específico ocorre: o botão esquerdo do mouse é clicado. Os procedimentos que estiverem estabelecidos dentro desta função serão executados assim que o evento for detectado.</p>

  <p>O click do mouse é dividido em 2 partes: um primeiro momento onde o botão é pressionado, cujos procedimentos são definidos em <a href="javascript:carrega('fun-mousePressed.php');"><code>mousePressed()</code></a>, um segundo momento onde o botão é liberado após o click. Se o tempo entre o pressionar e o liberar for muito rápido e a posição do mouse não se alterar, é invocada a função <code>mouseClicked()</code>; se o tempo entre o pressionar e o liberar for mais longo, ou houver uma mudança de posição, a função <a href="javascript:carrega('fun-mouseDragged.php');"><code>mouseDragged()</code></a> é invocada, e posteriormente a função <a href="javascript:carrega('fun-mouseReleased.php');"><code>mouseReleased()</code></a> quando o botão do mouse for liberado.</p>

  <p>Na ausência de <a href="javascript:carrega('fun-mouseReleased.php');"><code>mouseReleased()</code></a>, a função <code>mouseClicked()</code> será chamada, caso não haja alteração na posição do mouse.</p>
...
<h2>Entradas</h2>
  ...
  <p><code>mouseClicked()</code> não recebe nenhuma variável ou valor de entrada.</p>

  <a name="Retorno" class="indice">Retorno</a>
<h2>Retorno</h2>

  <p><code>mouseClicked()</code> é uma função do tipo <code>void</code>, e portanto não retorna nenhum valor.</p>
