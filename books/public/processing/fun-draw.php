...
?>
    ...
    <meta name="description" content="Draw | O código Processing, capítulo do livro O tecido das tecnologias criativas">
    <meta property="og:title" content="Draw | O tecido das tecnologias criativas">
    <meta property="og:description" content="Draw | O código Processing, capítulo do livro O tecido das tecnologias criativas">    
...
<h1 id='titulocapitulo'>draw()</h1>
  ...
  <p>A função <code>draw()</code> é parte, junto com a função <a href="javascript:carrega('fun-setup.php');"><code>setup()</code></a>, das duas funções auto-executadas do <span class="marca">Processing</span>, isto é, ambas são invocadas pelo próprio contexto da plataforma.</p>

  <p>A diferença entre as duas funções é que <a href="javascript:carrega('fun-setup.php');"><code>setup()</code></a> é invocada apenas uma única vez no ciclo de vida do programa, mais precisamente no primeiro momento que o código é executado, enquanto <code>draw()</code> é invocada continuamente, numa taxa de repetição determinada pela função <a href="javascript:carrega('fun-framerate.php');"><code>frameRate()</code></a>.</p>

  <p>O papel da função <code>draw()</code> é, como seu próprio nome diz, desenhar na tela. Contudo, ela acaba desempenhando papel muito mais importante: ela é o próprio fio-condutor de toda a execução de um programa em <span class="marca">Processing</span>. Conforme a função vai sendo continuamente executada pelo próprio contexto em que o programa está sendo executado, é <code>draw()</code> que vai acionando outras funções, e vai coordenando e tocando os processos do programa. De forma simples, <code>draw()</code> é o esqueleto central de todo progama desenvolvido em <span class="marca">Processing</span>, pela própria arquitetura da linguagem de programação.</p>
...
<h2>Retorno</h2>
  ...
  <p><code>draw()</code> é uma função do tipo <code>void</code>, e portanto não retorna nenhum valor.</p>
