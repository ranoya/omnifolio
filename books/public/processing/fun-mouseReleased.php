...
?>
    <title>mouseReleased | O tecido das tecnologias criativas</title>
    <meta name="description" content="mouseReleased | O código Processing, capítulo do livro O tecido das tecnologias criativas">
    <meta property="og:title" content="mouseReleased | O tecido das tecnologias criativas">
    <meta property="og:description" content="mouseReleased | O código Processing, capítulo do livro O tecido das tecnologias criativas">    
...
<body>
  ...
  <div id="texto">
  <a name="mouseReleased()" class="indice">mouseReleased()</a>
<h1 id='titulocapitulo'>mouseReleased()</h1>

  <p>A função <code>mouseReleased()</code> é uma função invocada pelo contexto do <span class="marca">Processing</span> quando um evento específico ocorre: o botão esquerdo do mouse é liberado após ser clicado. Os procedimentos que estiverem estabelecidos dentro desta função serão executados assim que o evento for detectado.</p>

  <p>O click do mouse é dividido em 2 partes: um primeiro momento onde o botão é pressionado, cujos procedimentos são definidos em <a href="javascript:carrega('fun-mousePressed.php');"><code>mousePressed()</code></a>, um segundo momento onde o botão é liberado após o click. Se o tempo entre o pressionar e o liberar for muito rápido e a posição do mouse não se alterar, é invocada a função <a href="javascript:carrega('fun-mouseClicked.php');"><code>mouseClicked()</code></a>; se o tempo entre o pressionar e o liberar for mais longo, ou houver uma mudança de posição, a função <a href="javascript:carrega('fun-mouseDragged.php');"><code>mouseDragged()</code></a> é invocada, e posteriormente a função <code>mouseReleased()</code> quando o botão do mouse for liberado.</p>

  <p>Na ausência de <code>mouseReleased()</code>, a função <a href="javascript:carrega('fun-mouseClicked.php');"><code>mouseClicked()</code></a> será chamada, caso não haja alteração na posição do mouse.</p>
...
<h2>Entradas</h2>
  ...
  <p><code>mouseReleased()</code> não recebe nenhuma variável ou valor de entrada.</p>

  <a name="Retorno" class="indice">Retorno</a>
<h2>Retorno</h2>

  <p><code>mouseReleased()</code> é uma função do tipo <code>void</code>, e portanto não retorna nenhum valor.</p>
